...
def getRoomList():
    try:
        ...
        if result:
            chatList=[{'chat-id':x['chat-id'],'owner':x['owner'],'participant':x['participant']}for x in result]
...
def getChat():
    try:
        ...
        info={'chat-id':chat['chat-id'],'owner':chat['owner'],'participant':chat['participant'],'chats':chat['chats']}
        if chat:
